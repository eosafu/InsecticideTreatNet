...
merged_data1_new<-select(merged_data1, "latitude", "longitude","v459","v460", "v461", "state", "v007", "v213")
...
combined_data1 <- combined_data1 %>%
  select(latitude, longitude, v459, v460, v461, state, v007, v213, REGNAME) %>%
  rename(
    latitude = latitude,
    longitude = longitude,
...
#select necessary columns
merged_data2_new<-select(merged_data2, "latitude", "longitude","v459","v460", "v461", "sstate", "v007", "v213")
...
combined_data2 <- combined_data2 %>%
  select(latitude, longitude, v459, v460, v461, sstate, v007, v213, REGNAME) %>%
  rename(
    latitude = latitude,
    longitude = longitude,
    mosquito_bednet = v459,
...
#2010
#Read the data
data3 <- read_dta("C:/Users/faith/OneDrive/Desktop/FAITH'S THESIS/DHS MALARIA/NG_2010_MIS_02032024_1922_197159/NGKR61DT/NGKR61FL.DTA")
...
coordinates_df3 <- data.frame(
  v001 = dhs_shapefile3$DHSCLUST,
  latitude = coordinates[, "Y"],
  longitude = coordinates[, "X"]  
)

#merge coordinates with data by clusters
merged_data3<- merge(coordinates_df3, data3, by = "v001")

#select necessary columns
merged_data3_new<-select(merged_data3, "latitude", "longitude", "v461", "sstate", "v007", "v213")

#merge new dataframe with reg_data
combined_data3 <- left_join(merged_data3_new, reg_data, by = c("sstate" = "REGCODE"))

#rename column names
combined_data3 <- combined_data3 %>%
  select(latitude, longitude, v461, sstate, v007, v213, REGNAME) %>%
...
#select necessary columns
merged_data4_new<-select(merged_data4, "latitude", "longitude","v459","v460", "v461", "sstate", "v007", "v213")
...
#select necessary columns
merged_data5_new<-select(merged_data5, "latitude", "longitude","v459","v460", "v461", "sstate", "v007", "v213")
...
#select necessary columns
merged_data6_new<-select(merged_data6, "latitude", "longitude","v459","v460", "v461", "sstate", "v007", "v213")
...
merged_data7_new <- merged_data7 %>%
  select(latitude, longitude, v459, v460, v461, v101, v007, v213, v002, v136, v001) %>%
  mutate(v101 = v101 * 10)
...
combined_data7 <- combined_data7 %>%
  select(latitude, longitude, v459, v460, v461, v101, v007, v213, REGNAME) %>%
  rename(
    latitude = latitude,
    longitude = longitude,
    mosquito_bednet = v459,
...
one <- select(combined_data1, latitude, longitude, respondent, pregnant,  year, state_code,regname)
two <- select(combined_data2, latitude, longitude, respondent,pregnant, year, state_code,regname)
three <- select(combined_data3, latitude, longitude, respondent,pregnant, year, state_code,regname)
four <- select(combined_data4, latitude, longitude, respondent,pregnant, year, state_code,regname)
five <- select(combined_data5, latitude, longitude, respondent,pregnant, year, state_code,regname)
six <- select(combined_data6, latitude, longitude, respondent, pregnant,year, state_code,regname)
seven <- select(combined_data7, latitude, longitude, respondent, pregnant, year, state_code,regname)


combined_data <- rbind(one, two, three, four, five, six, seven)
combined_data <- na.omit(combined_data)
pregnant_data <- combined_data %>%
  filter(pregnant == 1)

#Modelling
combined_data_coord <- as.matrix(select(pregnant_data, longitude, latitude))

# Convert combined_data to an sf object
combined_data_sf <- st_as_sf(pregnant_data, coords = c("longitude", "latitude"), crs = st_crs(Nigeria_sf))
...
combined_data_filtered <- combined_data_filtered[combined_data_filtered$respondent != 9, ]
...
Nigeria_sf <- st_read("C:/Users/faith/OneDrive/Desktop/Practice_sessions_on anemia_prevalence/Practice_sessions_on anemia_prevalence/gadm41_NGA_shp/gadm41_NGA_1.shp")
...
stk.e <- inla.stack(
  tag = "est",
  data = list(y = combined_data_filtered$respondent),
  A = list(1, A, 1),
...
res <- inla(formula,family="binomial", data = inla.stack.data(stk.full), control.predictor = list(compute = TRUE, A = inla.stack.A(stk.full)))

#time plot
time_effects <- res$summary.random$t
time_effects
time_df <- data.frame(
  year = unique(combined_data_filtered$year),
