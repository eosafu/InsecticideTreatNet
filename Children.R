...
merged_data1_new<-select(merged_data1, "latitude", "longitude","v459","v460", "v461", "state", "v007")
...
combined_data1 <- combined_data1 %>%
  select(latitude, longitude, v459, v460, v461, state, v007, REGNAME) %>%
  rename(
    latitude = latitude,
    longitude = longitude,
...
#select necessary columns
merged_data2_new<-select(merged_data2, "latitude", "longitude","v459","v460", "v461", "sstate", "v007")
...
combined_data2 <- combined_data2 %>%
  select(latitude, longitude, v459, v460, v461, sstate, v007, REGNAME) %>%
  rename(
    latitude = latitude,
    longitude = longitude,
    mosquito_bednet = v459,
...
#select necessary columns
merged_data3_new<-select(merged_data3, "latitude", "longitude", "hv227", "hv228", "hv024", "hv007")
...
combined_data3 <- combined_data3 %>%
  select(latitude, longitude, hv227, hv228, hv024, hv007, REGNAME) %>%
...
#select necessary columns
merged_data4_new<-select(merged_data4, "latitude", "longitude","v459","v460", "v461", "sstate", "v007")
...
#select necessary columns
merged_data5_new<-select(merged_data5, "latitude", "longitude","v459","v460", "v461", "sstate", "v007")
...
#select necessary columns
merged_data6_new<-select(merged_data6, "latitude", "longitude","v459","v460", "v461", "sstate", "v007")
...
#select necessary columns
merged_data7_new<-select(merged_data7, "latitude", "longitude","v459","v460", "v461", "v101", "v007")
...
combined_data7 <- combined_data7 %>%
  select(latitude, longitude, v459, v460, v461, v101, v007, REGNAME) %>%
  rename(
    latitude = latitude,
    longitude = longitude,
    mosquito_bednet = v459,
...
one <- select(combined_data1, latitude, longitude, children_under5, year, state_code,regname)
two <- select(combined_data2, latitude, longitude, children_under5, year, state_code,regname)
three <- select(combined_data3, latitude, longitude, children_under5, year, state_code,regname)
four <- select(combined_data4, latitude, longitude, children_under5, year, state_code,regname)
five <- select(combined_data5, latitude, longitude, children_under5, year, state_code,regname)
six <- select(combined_data6, latitude, longitude, children_under5, year, state_code,regname)
seven <- select(combined_data7, latitude, longitude, children_under5, year, state_code,regname)

combined_data <- rbind(one, two, three, four, five, six, seven)
combined_data <- na.omit(combined_data)

#Create binary values for children under 5 who slept under bednet
combined_data$new_children_under5 <- ifelse(combined_data$children_under5 == 0 | combined_data$children_under5 == 3, 0, 1)
...
stk.e <- inla.stack(
  tag = "est",
  data = list(y = combined_data_filtered$new_children_under5),
  A = list(1, A, 1),
