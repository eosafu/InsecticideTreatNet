...
#select necessary columns
merged_data3_new<-select(merged_data3, "latitude", "longitude", "hv227", "sh09_02", "hv228", "hv024", "hv007")
...
combined_data3 <- combined_data3 %>%
  select(latitude, longitude, hv227, hv228, hv024, hv007, sh09_02, REGNAME) %>%
...
one <- select(combined_data1, latitude, longitude, mosquito_bednet, pregnant,  year, state_code,regname)
two <- select(combined_data2, latitude, longitude, mosquito_bednet,pregnant, year, state_code,regname)
three <- select(combined_data3, latitude, longitude, mosquito_bednet,pregnant, year, state_code,regname)
four <- select(combined_data4, latitude, longitude, mosquito_bednet,pregnant, year, state_code,regname)
five <- select(combined_data5, latitude, longitude, mosquito_bednet,pregnant, year, state_code,regname)
six <- select(combined_data6, latitude, longitude, mosquito_bednet, pregnant,year, state_code,regname)
seven <- select(combined_data7, latitude, longitude, mosquito_bednet, pregnant, year, state_code,regname)
...
combined_data_filtered <- combined_data_filtered[combined_data_filtered$mosquito_bednet != 9, ]
...
stk.e <- inla.stack(
  tag = "est",
  data = list(y = combined_data_filtered$mosquito_bednet),
  A = list(1, A, 1),
